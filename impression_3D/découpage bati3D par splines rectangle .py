...
"""sélectionner l'objet polygonal avec les bâtiment ou les arbres
   Attention mettre l'axe à zéro'
...
def cut_mnt_by_rectangle(mnt,sp,delta = 200) -> None:
    ...
    if res:
        ...
        resobj = res[0]
        if resobj:
            resobj.SetName(f'{mnt.GetName()}_{sp.GetName()}')
...
def main() -> None:
    ...
    if not bats or not bats.CheckType(c4d.Opolygon):
        ...
        return
    splines = [sp for sp in op.GetNext().GetChildren() if sp.CheckType(c4d.Ospline)]
    if not splines:
